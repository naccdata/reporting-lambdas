...
**Note**: The `VisitEvent` model supports four event types (`submit`, `delete`, `not-pass-qc`, `pass-qc`), but currently only `submit` and `pass-qc` events are actively logged by the system.
...
- Lowercase
- Defaults to `"adrc"` if not explicitly set
- Extracted from project label suffix: `ingest-{datatype}-{study}` → `study="{study}"`
- For ADRC projects (no suffix): `ingest-{datatype}` → `study="adrc"` (default)
- Examples:
  - `ingest-form` → `study="adrc"`
  - `ingest-form-dvcid` → `study="dvcid"`
  - `ingest-dicom-leads` → `study="leads"`
...
**Note**: The `study` field is extracted from the project label. For ADRC projects (`ingest-form`), it defaults to `"adrc"`.
...
**Note**: The `study` field is extracted from the project label suffix: `ingest-form-dvcid` → `study="dvcid"`.
...
**Note**: The `study` field is extracted from the project label suffix: `ingest-dicom-leads` → `study="leads"`. For DICOM data, the `module` and `packet` fields are omitted.
...
File: `s3://nacc-events/prod/log-submit-20240115-100000-42-ingest-form-110001-2024-01-15.json`
...
File: `s3://nacc-events/prod/log-pass-qc-20240115-102000-42-ingest-form-110001-2024-01-15.json`
...
- Documents `submit` and `pass-qc` event types
- Flat S3 storage structure with environment prefixes
- Event filename format: `log-{action}-{timestamp}-{adcid}-{project}-{ptid}-{visit_date}.json`
- Note: `delete` and `not-pass-qc` event types are defined in the model but not currently logged
